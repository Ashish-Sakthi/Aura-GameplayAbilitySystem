...
void UAuraAbilitySystemComponent::ServerEquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
   if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
   {
      ...
      if (bStatusValid)
      {
         // Remove the target slot tag from other abilities (so it's unique)
         ClearAbilitiesOfSlot(Slot);
         // Remove this ability's previous slot (so it isn't assigned to two slots)
         ClearSlot(AbilitySpec);
         // Assign this slot to this ability
         AbilitySpec->DynamicAbilityTags.AddTag(Slot);
         // If not equipped already, set to equipped
